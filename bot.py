...
logging.basicConfig(filename=r'.\output.log', filemode='w', level=logging.INFO, format='%(asctime)s:%(levelname)s:%(message)s')

codex = commands.Bot(command_prefix = '!')

@codex.event
async def on_ready():
    await codex.change_presence(status=discord.Status.online,activity=discord.Game('5th Edition D&D'))
...
async def reload(ctx, extension):
    """
    Reloads a single cog. I guess use it if you are having trouble and want to restart it?
    """
    codex.unload_extension(f'cogs.{extension}')
    codex.load_extension(f'cogs.{extension}')
...
## Assigned as an environment variable on the server. Can be found in discordapp.com/developer for the bot.
